...
Reasoning benchmarks are useful only to the extent that success transfers beyond the examples, templates, domains, and difficulty levels used to build a model. This requirement is stricter than held-out accuracy. A model may answer unseen items from a familiar template while failing after a symbol renaming, a change in wording, an extra inference step, a new language, or a new tool interface. Controlled studies expose this gap in arithmetic, formal languages, graph planning, scientific composition, analogy, and multimodal reasoning. GSM-Symbolic finds large variance across numerical instantiations of the same template and sharp failures after irrelevant clauses are added ([Mirzadeh et al., 2024](https://arxiv.org/abs/2410.05229)); MathGAP finds accuracy declines as arithmetic proof trees become deeper, wider, and nonlinear ([Opedal et al., 2024](https://arxiv.org/abs/2410.13502)); and XDomainBench reports a collapse as scientific domains are composed ([Cao et al., 2026](https://arxiv.org/abs/2605.14754)). These failures motivate a survey centered on transfer, not benchmark score alone.
...
**Review protocol.** The literature search was updated on 10 September 2026. Searches covered reasoning generalization, out-of-distribution reasoning, systematic and compositional generalization, length and depth generalization, reasoning transfer, and test-time generalization. Targeted abstract-field audits combined training-stage terms with generalization, transfer, unseen-combination, symbolic-variant, longer-input, deeper-proof, and OOD terms. Queries were split by submission period so that the larger 2026 literature did not crowd out 2024 and 2025 records. After deduplication, we screened 6,012 arXiv records and checked relevant ACL Anthology and OpenReview entries. A second pass read every retained abstract and inspected the full paper whenever the abstract did not identify the evaluation split. Every core study required four identifiable elements: reasoning task, reference training exposure, test shift, and evidence of transfer or failure. We excluded ordinary reasoning-score improvements, broad robustness studies in which reasoning was only one task type, model-to-model transfer without an unfamiliar-problem shift, and expressivity results with no generalization claim. The final evidence corpus contains 145 core studies: 86 first posted to arXiv in 2026, 39 in 2025, 14 in 2024, and 6 earlier foundations. Four nearby surveys and two contextual comparisons are not counted as core evidence. The 151-source machine-readable manifest includes titles, authors, abstracts, taxonomy labels, source URLs, and local PDF paths in [`paper_manifest.tsv`](data/paper_manifest.tsv); audit exclusions and reasons are recorded in [`excluded_papers.tsv`](data/excluded_papers.tsv).

**Relation to nearby surveys.** Existing reviews cover broader NLP generalization ([Hupkes et al., 2023](https://arxiv.org/abs/2210.03050)), inductive reasoning ([Survey of Inductive Reasoning, 2026](https://arxiv.org/abs/2510.10182)), length extrapolation in algorithmic tasks, reinforced reasoning, or generalizability of LLM agents ([Agent Generalizability Survey, 2026](https://arxiv.org/abs/2509.16330)). This survey differs by treating training, inference, architecture, behavior, mechanisms, and theory as parts of one question: whether a reasoning procedure survives a stated shift. Evidence is organized by scientific role rather than publication year, with the 2024 and 2025 foundations used to interpret the larger 2026 wave.
...
[The Imitation Game](https://arxiv.org/abs/2507.13332) gives a controlled 2025 example of this strategy. It synthesizes chain-of-thought traces from Turing-machine executions across eight algorithm classes and tests inputs longer than those used for training. Its strongest result is not generic synthetic-data scaling, but transfer of explicit state-transition traces across several algorithm families. The limitation is equally important: the trace language and task programs are known, so the result does not establish that web-scale language models will discover comparable state decompositions without such supervision.
...
[Learning Composable Chains-of-Thought](https://arxiv.org/abs/2505.22635) isolates the supervision target more directly. Models learn chain-of-thought traces for atomic tasks, while the target composition has no labeled reasoning trace. A composable trace format, multi-task training or model merging, and a small amount of target reinforcement fine-tuning allow the learned steps to be reused on unseen compositions. This supports supervising interfaces between skills, not only complete solutions.
...
Two 2025 studies make the recombination claim concrete. [From $f(x)$ and $g(x)$ to $f(g(x))$](https://arxiv.org/abs/2509.25123) controls contamination with synthetic string transformations: RL learns unseen compositions of known atomic functions, extrapolates to more than two functions, and transfers the composition procedure to a different task whose primitives were already known. [LogicPuzzleRL](https://arxiv.org/abs/2506.04821) trains with verifiable rewards across seven logic-puzzle families and reports transfer of algebraic, geometric, and combinatorial routines to held-out mathematical benchmarks, while transfer is weaker for rote or highly specialized skills. Together they suggest that RL transfer depends on reusable primitives and compatible composition structure.

Reward granularity can change which procedure is selected. [GraphPRM](https://arxiv.org/abs/2503.00845) constructs step-level labels from graph structure, uses them for process-reward modeling and policy optimization, and tests transfer to unseen graph datasets as well as GSM8K and MATH-500. The cross-family results are evidence for a reusable checking signal, although graph-derived supervision remains a strong structural prior.
...
Self-improvement changes model parameters or training data using prior solutions and failures. [Self-Improving Transformers](https://arxiv.org/abs/2502.01612) iteratively trains on self-generated correct solutions and reports transfer from 10-digit to 100-digit arithmetic, together with easy-to-hard gains in string manipulation and maze tasks. Because correctness is externally checkable, the result isolates a useful loop: generate beyond the current supervised range, verify, and convert successful exploration into new training data. [STEPS](https://arxiv.org/abs/2601.03676) constructs a skill taxonomy, synthesizes underrepresented skill combinations, and evaluates compositional transfer in downstream agent tasks. [CALM](https://arxiv.org/abs/2607.23771) trains language models with several inference controllers and tests held-out controller compositions. These studies define what is learned during training and what combination or difficulty changes at test time.
...
- [Exploring Depth Generalization](https://arxiv.org/abs/2512.02677) separates training and test recursion depth on Boolean, arithmetic, and logic tasks. Standard transformers degrade sharply beyond the training depth, while a looped locator-and-replacer design transfers better on several tasks.
...
Length generalization is partly a problem of positional distribution shift. Learned absolute positions leave unseen indices unconstrained, while relative schemes can reuse a learned relation at new positions. Two 2024 arithmetic studies show how task structure can repair this mismatch. [Position Coupling](https://arxiv.org/abs/2405.20671) aligns the positions of digits that participate in the same operation and reports addition transfer from at most 30 training digits to 200 test digits. [Explicitly Encoding Structural Symmetry](https://arxiv.org/abs/2406.01895) combines reversed number formatting with a custom position scheme and extrapolates from 5-digit training examples to 50-digit addition. These are narrow tasks, but they identify a concrete design principle: coordinates should represent the operation's symmetry rather than incidental token indices.
...
Structured scratchpads can play a similar role without changing the attention operator. [Arithmetic Transformers Can Length-Generalize](https://arxiv.org/abs/2410.15787) combines task-specific intermediate traces with multi-level position coupling and tests both longer operands and more operands than appear in training. The result strengthens the case that a scratchpad is useful when its state and coordinates make the repeated local operation explicit.
...
A model can generalize along one axis and fail along another. [GSM-Plus](https://arxiv.org/abs/2402.19255) shows that models can solve an original GSM8K item yet fail when a new statement is added or the question target changes. [Rule Extrapolation](https://arxiv.org/abs/2409.13728) tests formal-language prompts outside the learned rule regime and finds that apparent reasoning competence depends strongly on whether the prompt remains compatible with training rules. In a controlled shortest-path setting, models transfer to unseen maps but fail when paths require more recursive steps ([Shortest Path Generalization, 2026](https://arxiv.org/abs/2604.15306)). A single OOD average hides these distinctions.
...
Surface changes are attractive because they preserve the underlying answer while removing familiar cues. [GSM-Symbolic](https://arxiv.org/abs/2410.05229) varies numerical instantiations generated from the same templates and adds clauses that should not affect the solution, revealing both high variance and severe distractor sensitivity. [GRAIN](https://arxiv.org/abs/2608.27142) varies graph node names and narrative form. [Shared Circuits](https://arxiv.org/abs/2609.04463) compares numeric and verbal arithmetic across languages. [When Symbol Names Should Not Matter](https://arxiv.org/abs/2605.07120) studies classification with fresh vocabularies. These studies show that symbol and format invariance cannot be assumed even for simple operations.
...
Compositional failure appears in language, math, science, graphs, space, and tools. [MathGAP](https://arxiv.org/abs/2410.13502) controls arithmetic proof-tree structure and finds larger drops for deeper, wider, and nonlinear proofs. [CGGC](https://arxiv.org/abs/2410.06272) holds graph relation types familiar but tests unseen combinations, finding that graph structure predicts difficulty and easy-to-hard demonstration order can help. [Compositional-ARC](https://arxiv.org/abs/2504.01445) tests unseen combinations of familiar spatial transformations; a small meta-learned encoder-decoder generalizes more systematically than the compared frontier LLMs. [XDomainBench](https://arxiv.org/abs/2605.14754) reports a related collapse as scientific composition order rises and interactive errors accumulate. [Why Knowing Both Hops Is Not Enough](https://arxiv.org/abs/2608.07261) shows that possession of the two atomic facts does not ensure two-hop transfer. Controlled post-training studies refine this pattern: [RL Post-Training Builds Compositional Reasoning Strategies](https://arxiv.org/abs/2607.07646) observes reuse of learned sequential and parallel rewrite procedures, while [Skill Composition on Countdown](https://arxiv.org/abs/2512.01775) finds persistent failure on particular expression-tree shapes even when depth is held constant.
...
Length can mean more tokens, more entities, more reasoning steps, or a longer output. These variables often correlate in training but require different mechanisms. [Exploring Length Generalization](https://arxiv.org/abs/2207.04901) showed that ordinary fine-tuning is weak on longer algorithmic inputs and that scratchpads can help. [Transformers Can Achieve Length Generalization But Not Robustly](https://arxiv.org/abs/2402.09371) adds an important qualification: formatting and positional choices can produce short-to-long arithmetic transfer, yet outcomes remain fragile across seeds and training order. [Exploring Depth Generalization](https://arxiv.org/abs/2512.02677) separately varies recursive depth and shows that longer visible traces and deeper recursive computation are not interchangeable. [Length Generalization Transfer](https://arxiv.org/abs/2506.09251) asks whether extrapolation learned on one task transfers to another.
...
### 6.1.7 Representative diagnostic suite

| Diagnostic | Primary shift | Main observation | Interpretation limit |
| --- | --- | --- | --- |
| [GSM-Plus](https://arxiv.org/abs/2402.19255) | Statements and answer targets | Correctness is brittle even on variants of solved problems | Perturbations remain within grade-school arithmetic |
| [GSM-Symbolic](https://arxiv.org/abs/2410.05229) | Numeric instantiation and distractor clauses | Equivalent templates yield unstable accuracy | Template generation cannot test every mathematical skill |
| [MathGAP](https://arxiv.org/abs/2410.13502) | Proof-tree depth, width, and shape | Accuracy declines with structural complexity | Generated word problems simplify natural language variation |
| [Compositional-ARC](https://arxiv.org/abs/2504.01445) | Unseen transformation combinations | Meta-learned small models can exceed frontier LLM systematicity | Abstract spatial tasks are controlled but narrow |
...
Controlled composition studies identify two more reusable computations. [Compositional Generalization from Learned Skills via CoT Training](https://arxiv.org/abs/2502.04667) finds that chain-of-thought training can form a two-stage circuit that first solves component tasks and then composes their outputs on held-out combinations. [Characterizing Pattern Matching and Its Limits](https://arxiv.org/abs/2505.20278) formalizes pattern matching through functional equivalence and shows that transfer depends on how many contexts witness the relevant equivalence. It also identifies path ambiguity as a structural failure mode that chain-of-thought reduces in sample cost but does not remove. These results separate useful circuit reuse from a stronger claim of unrestricted systematic reasoning.
...
[On Provable Length and Compositional Generalization](https://arxiv.org/abs/2402.04875) supplies an earlier cross-architecture account. Under controlled data and capacity assumptions, it shows that length and compositional transfer depend on both training diversity and restrictions that prevent memorizing arbitrary finite behavior; chain-of-thought supervision can improve the regime in which higher-capacity models generalize. The result links representation, capacity, and data coverage rather than attributing extrapolation to architecture alone.
